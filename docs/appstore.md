...
## 11-4. Google Play への出し方（v79-v82 = versionCode 10 / 1.3・2026-08-13）

**🚨 なぜ Play 経由なのか（この回で分かったこと）**
公開中のアプリは **Play から入れた＝Google Play の署名**が付いている。ローカルで焼いた APK は開発用の署名なので、**上書きインストールが Android に拒否される**（`INSTALL_FAILED_UPDATE_INCOMPATIBLE`）。消してから入れれば入るが、**端末内のデータ（リスト台帳・辞書・詳細設定・API キー・来歴）は全部失われる**——Play 版は `run-as` が使えず、**こちらからバックアップを取る手段が無い**。
→ **公開済みのアプリを実機で更新する道は Play（内部テスト）だけ**。ローカル APK は「まだ Play に出していない機能を、データを捨ててよい端末で試す」時の道具。
※ 並べて入れる道（`applicationIdSuffix ".debug"`）も可能だが、`shortcuts.xml` の `targetPackage` が固定なので**ホーム長押しだけ debug 用の上書きが要る**（`android/app/src/debug/res/xml/shortcuts.xml`）。

**焼いてある成果物（このリポジトリで再現できる）**
```
android/app/build/outputs/bundle/release/app-release.aab   ← Play へ上げるのはこれ（署名済み・3.14MB）
android/app/build/outputs/apk/debug/app-debug.apk          ← adb install 用（Play 版とは共存できない）
```
再ビルド:
```
npm run sync:web ; npx cap copy android ; ./android/gradlew.bat -p android bundleRelease
```
（`$env:JAVA_HOME="C:\Program Files\Android\Android Studio\jbr"` が要る。署名鍵はリポ外 `~/Documents/voice-calendar-signing/upload-keystore.properties`）

**検分済み（2026-08-13）**: `jarsigner -verify` exit 0 ／ merged manifest `versionCode=10` `versionName=1.3` ／ **AAB 同梱の web が `BUILD=v82`・`?v=82` ×13・`ovKeepOpen` 在り**。
⚠ **gradle は `UP-TO-DATE` を並べても成果物は新しいことがある（逆もある）＝必ず AAB の中身を開いて確かめる**（v10「版表示に嘘をつかせない」）。

**Play Console で触る順**
1. **テスト → 内部テスト → 新しいリリースを作成**
2. `app-release.aab` をアップロード（versionCode 10 は初出＝重複エラーは出ない）
3. **リリースノート**（`<ja-JP>` に貼る）← 下記
4. 保存 → **リリースのレビュー** → **内部テストへの公開を開始**
5. 端末の Play ストア → ボイスカレンダー → 更新（反映まで数分〜十数分。出ない時は Play ストアのキャッシュ削除か、内部テストの参加リンクを踏み直す）

**リリースノート（内部テスト・v1.3）**
```
・長い話を最後まで: 録音中の「この録音だけ 止めない」を押すと、話し終わっても自動で止まりません。思ったことを長文で吐き出したい時に。マイクをもう一度押すと確定します（最長10分）。
・長い文章を整える: メモが長い時に「文章を整える」が出ます。認識ミス・句読点・言い淀みだけを AI が直します（内容は足しません。来歴の ↩ でいつでも元に戻せます）。※お使いの API キーの登録が要ります
・つぶやいた場所の地図: 保存した記録の位置を1枚の地図にまとめて見られます。地図を開いた時だけ地図画像を取得し、座標や記録の内容は送信しません。
・修正: ホーム画面の長押しで「リスト」を開いた時、録音画面が上に出てリストが見えなくなっていました。
```
