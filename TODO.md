...
>
> **▶▶ 次回はここから＝Codemagic で v13 をビルド → TestFlight → 実機で再確認**:
> ① まず**全ボタンが動くか**（🎤・発話として送る・保存・クリア）＋フッタが `v13：…`
> ② 「診断」を開いて **環境=native (Capacitor) / 音声エンジン=sfspeech** になっているか（＝Swift プラグインが登録されている証拠。`webspeech` に落ちていたら native 失敗理由がそこに出る）
> ③ **無音1.8秒の自動確定の手触り**（ノールックの生命線・実機でしか分からない）
> ④ **保存 → 本物のカレンダーに入るか**（＝v0 の一本道の完成）／OS の既定が Google なら Google にも出るか
>
