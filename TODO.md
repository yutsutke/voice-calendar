...
>
> **▶▶ 次回はここから＝Codemagic で v17 をビルド（www 同梱のため要リビルド・Swift 変更なし）→ 実機で**:
> ① 欄指定の手触り: 予定を話す →「終了22時」「場所 立川」「メモ ○○」が**積み上がる**か（消えないか）
> ② **保存 → 本物のカレンダーアプリに入るか**（一本道の最終確認・EventKit の初実走）／OS の既定が Google なら Google にも出るか
