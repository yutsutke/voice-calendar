...
>
> **🔧 v7（実発話FB第3回）**: 「時間にひっぱられる」再報告 → ローカル v6 で再現せず・症状は v5 と一致＝**Pages の10分キャッシュで旧版を見ていた**と切り分け（📌 FB を受けたらまずフッタの BUILD 版を確認してもらうこと）。スクショから本物のバグ発見: **「N月のN日」の「の」未対応で月が黙って無視**（「12月の5日」→8/5 に化ける事故を修正・テスト固定）。**テスト 71/71 ✅**。
>
