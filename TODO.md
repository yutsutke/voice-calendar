...
>
> **▶▶ 次回はここから＝Codemagic で v15 をビルド → 実機で再発話**（「7月の28日10時美術館」「2026年7月28日10時美術館」「昨日10時美術館」＋好きな発話）:
> ・**今度はどんな発話も必ず入るはず**（正規確定 or 補完確定）
> ・来歴の「補完確定」の割合を見る＝iOS の isFinal がどれだけ壊れているかの実データ
> ・診断の 🎙 行（開始→無音→isFinal or タイムアウト）で Swift の動きが読める
> ・全部入るようになったら → **v0 一本道の完成確認**（保存→本物のカレンダー→Google 同期）→ ノールック完走率の計測へ（SPEC §10）
>
