...
>
> **▶▶ 次回はここから＝Codemagic で v16 をビルド → 実機で「録音ボタンを切らずに」発話**（これが本来のノールック経路）:
> ・「7月の28日10時美術館」「2026年7月28日10時美術館」「昨日10時美術館」＋好きな発話 → **今度は自動でも入るはず**
> ・診断の 🎙「確定 len=N partial=M」で**空の確定が実際どれだけ起きているか数字で分かる**（→ 多いなら無音1.8秒の調整や設計見直しの材料）
> ・来歴の「補完確定」印の割合＝正規の確定がどれだけ機能しているか
> ・入るようになったら → **v0 一本道の完成確認**（保存→本物のカレンダー→Google 同期）→ ノールック完走率の計測へ（SPEC §10）
>
