...
>
> **🔧 v8（実発話FB第4回・重要な原則変更）**: 「6月の30日が6月30日にならない」→ **「過去なら来年」の未来固定ルールを廃止**し、**「言っていない年・月は今日に最も近いものを選ぶ」**に統一（過去も一級市民＝v5 の学びを絶対日付にも徹底）。存在しない日付（2月30日）は blocked スパンで素通し＋理由表示（素のN日が拾って 7/30 を創作するバグをテストが発見）。**テスト 76/76 ✅**（parser 62 + schema 14）。
>
...
- **年の中間帯（v8）**: 「3月1日」＝4か月前 or 8か月先、はどのルールでも当たらない（現在は「最も近い」＝今年3/1）。**v1 の `guessed`（仮＝淡色/点線）の第一候補**——「一番近いのを入れたけど仮だよ」と見せて直させるのが正解の形。実際に外して困ったかを観察する。
