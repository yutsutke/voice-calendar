...
>
> **▶▶ 次回はここから＝Codemagic で v14 をビルド → 実機で「7月の28日10時美術館」「2026年7月28日」「昨日10時美術館」を再発話**。判定表:
> ・入るようになった → 不可視文字が真因（クローズ）
> ・来歴に 🛑 → エラー内容がそのまま見える
> ・来歴に出るのに入らない → 🗣 の生テキストが真実を教える（漢数字等）→ その文字列がテストケースになる
> ・来歴に出ない → final イベント未達＝Swift 側（無音タイマー/isFinal）を疑う
>
