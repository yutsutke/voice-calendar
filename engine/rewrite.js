...
(function (global) {
  ...
  // 🆕 v96: **逐次モード**（ゆう要求 2026-08-30「話しながら要約や文章の揺れを直してくれるモード」
  //   「息継ぎや数語話すたびに前の文章からの意味を察してくれて、言い間違えや変換のブレにたいして
  //   ＡＩが自動で訂正してくれる」）。区切り方の数字は engine が持つ（宿主に数字を書かない）。
  const LIVE = {
    MIN_CHUNK_CHARS: 8,    // これ未満の断片は区切らない（「はい」だけを AI に投げない＝1呼び出しの意味を保つ）
    PAUSE_MS: 900,         // 途中結果がこれだけ止まったら「息継ぎ」とみなして区切る（ゆうの言葉の実装）
    MAX_TAIL_CHARS: 120,   // 息継ぎが無くてもこれだけ溜まったら区切る（区切りが来ないまま溜まり続けない）
    CONTEXT_CHARS: 100,    // 訂正に添える直前の文脈の長さ（「前の文章からの意味を察して」の実装）
  };
  ...
  const MODES = {
    ...
    // 🆕 v96 逐次モードの「訂正」: 話している最中の**断片**を、直前の文脈を手がかりに直す。
    // tidy との違いは相手が**文の途中で切れた断片**であること＝①完成した文に整えようとしない
    // ②文脈は手がかりであって出力の一部ではない、の2点を指示で明確に縛る。
    correct: {
      label: '訂正',
      done: '訂正しました',
      min: 0.5,
      max: 1.6,   // 句読点を足すと少し伸びる（tidy と同じ理屈・断片は短いぶん比が振れやすいので上限は少し広め）
      tooShort: (b, t) => `短くなりすぎました（${b}字→${t}字）＝削られた可能性があるので当てません`,
      tooLong: (b, t) => `長くなりすぎました（${b}字→${t}字）＝書き足された可能性があるので当てません`,
      // ctx = 直前の（すでに訂正済みの）文章の末尾。無ければ文脈ブロックごと省く。
      prompt: (ctx) => [
        'あなたは日本語の音声認識の結果を直す校正者です。いま話している途中の断片が届きます。',
        '',
        ...(ctx ? ['【直前の文章（手がかり。出力に含めない）】', String(ctx), ''] : []),
        '【必ず守ること】',
        '- 届いた断片だけを直して返す。直前の文章を出力に含めない。',
        '- 断片は断片のまま返す。文を完成させない・続きを推測して足さない。',
        '- 内容を足さない・減らさない。要約しない。',
        '- 事実（人名・地名・数字・日付・時刻・金額）を変えない。確信が無ければそのままにする。',
        '',
        '【直してよいこと（これだけ）】',
        '1. 音声認識の変換ミス（同音異義語）を、直前の文章と断片の文脈から明らかな場合だけ直す',
        '2. 句読点（、。）を入れて文を区切る（断片の末尾が文の切れ目なら句点を付けてよい）',
        '3. 言い淀み（えー、あの、えっと、まあ）を消す',
        '4. 同じことを言い直している箇所は、後の言い方を残して重複を消す',
        '',
        '【出力】',
        '直した断片だけを返す。前置き・説明・引用記号（```）を付けない。',
        '直すところが無ければ、受け取った断片をそのまま返す。',
      ].join('\n'),
    },
  };
  ...
  function buildPrompt(mode, ctx) { return modeOf(mode).prompt(ctx); } // ctx は correct だけが読む（他は無視）
  ...
  const api = {
    MIN_CHARS, MIN_RATIO, MAX_RATIO, REVIEW_MIN_CHARS, AUTO_SUMMARY_CHARS, LIVE, MODES,
