...
(function (global) {
  ...
  const DEFS = [
    ...
    },
    {
      key: 'voiceAI',
      label: '音声を AI で解釈する',
      hint: 'オン: 欄名で始まらない発話を、登録した API キー（「まとめて入力」の AI 設定）で解釈する。長く自然な言い方に強くなり、複数の予定を読み取った時は「まとめて入力」のリストに入る。発話のたびに API を1回呼ぶ（少額の従量課金・応答まで数秒）。AI が失敗した時はいつものルール解釈に自動で戻る。AI で解釈した発話は自動保存されない（フォームに残るので保存を押す）。オフ: 今までのルール解釈（オフライン・即時・無料）。',
